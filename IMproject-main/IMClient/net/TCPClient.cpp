...
bool TCPClient::sendData(char* data, int len, u_long to)
{
	...
	if (len > MAX_PACK_LEN)
	{
		cout << "TcpClient::sendData len over MAX_PACK_LEN:" << len << endl;
		return false;
	}

	// 2、先发包长度len（统一转网络字节序大端，跨平台一致）
	int netLen = htonl(len);
	int nSendNum = send(m_socket, (char*)&netLen, sizeof(netLen), 0);
...
void TCPClient::recvData()
{
	...
	while (m_bRunning) {
		...
		if (nRecvNum > 0)
		{
			...
			// 1.2 长度上限保护（防止异常/恶意超大 RecvLen 导致 OOM）
			if (RecvLen <= 0 || RecvLen > MAX_PACK_LEN)
			{
				cout << "TCPClient::recvData illegal RecvLen:" << RecvLen << endl;
				break;
			}
