...
### 阶段 0.5：功能补齐（新增，2026-08-13 决策）

> **决策背景**：djinni IDL 一旦生成三端绑定，`ClientCore`/`IClientEvents` 的 API 变动会放大为 N 端返工。因此把会改变 API 表面的功能提前到"只有 Qt 一个薄 UI"的低成本窗口期完成，待 API 收敛后再进阶段 1。
>
> **顺序与理由**：心跳保活（打地基，在线状态准确性前提）→ 消息漫游（验证本地存储 schema + 分页拉取协议）→ 群聊（确立"会话 + 消息类型"模型，API 收敛最后一块）→ 阶段 1。富媒体传输实现放阶段 1 之后（仅 Qt+Electron 两端时改动仍可控），但会提前在 `.proto` 中预留 `msg_type` 字段占位。
>
> **验证闭环**：Qt + VS 服务端（Windows）+ client_core 测试（macOS）。

- [ ] **心跳保活**（1~2 天）：客户端定时心跳 + 超时判定断连；服务端活跃时间扫描 + 超时强制下线并广播好友
- [ ] **消息漫游**（2~3 天）：服务端全量消息表 + 按时间戳分页拉取协议 + 本地/服务端历史合并
- [ ] **群聊**（3~5 天）：群消息/成员管理协议 + 服务端扇出 + 群表/群成员表 + 会话列表 UI
- [ ] `.proto` 预留 `msg_type`（text/image/file）占位字段（随群聊一并完成）
